...
class Checkin_Scan extends CI_Controller {
	...
	public function index()
	{
        ...
        $query = "EXEC dbo.sp_tnc_ticket_trans_checkin_scan_load ?, ?, ?, ?";
		...
		$this->load->view('Tenancy/Ticketing/Trans/Checkin_Scan/index', $data);
    ...
    public function create($building_code, $venue_code, $gate_code)
	{
        ...
        $this->load->view('Tenancy/Ticketing/Trans/Checkin_Scan/create', $data);
	}
    ...
    public function destroy($checkin_id, $ticket_id, $seq, $gate_id)
	{
        $this->db->trans_begin();
        try {
            $this->Checkin_Scan_model->delete($checkin_id, $ticket_id, $seq, $gate_id);

            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                
                is_pesan('error','Delete failed, please try again.');
                redirect('Tenancy/Ticketing/Trans/Checkin_Scan/index');
            } else {
