...
class Venue extends CI_Controller {
	...
	{
		$data['judul'] = 'Locations';
		$data['bodyclass'] = 'skin-blue';

		$data['results'] = $this->Venue_model->get_all();
        // relation
        foreach ($data['results'] as $p) {
            // belongsTo
            $p->building = $this->Venue_model->building($p->area);
        }

		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('Tenancy/Ticketing/Master/Venue/index', $data);
    ...
    public function create()
	{
		...
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('Tenancy/Ticketing/Master/Venue/create', $data);
		$this->load->view('templates/footer');
	}
    ...
    public function store()
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			redirect('Tenancy/Ticketing/Master/Venue/create');
        }

        $this->db->trans_begin();
        try {
            $exists = $this->Venue_model->check_exists(
                [
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                redirect('Tenancy/Ticketing/Master/Venue/index');
            }
        } catch (Exception $e) {
            $this->db->trans_rollback();
	...
	{
		$data['judul'] = 'Edit Location';
		$data['bodyclass'] = 'skin-blue';
		
        $data['result'] = $this->Venue_model->find($id);
		$data['building'] = $this->Building_model->get_by_is_active(1);

		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
		$this->load->view('Tenancy/Ticketing/Master/Venue/edit', $data);
		$this->load->view('templates/footer');
	}
    ...
    public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			is_pesan('error','Unfornuately, something went wrong.');
			redirect('Tenancy/Ticketing/Master/Venue/edit/'.$id);
        }
