...
class Outgoing extends CI_Controller {
	public function __construct()
	{
		...
		$this->load->model('Tenancy/Inventory/Stock/Outgoing_model');
		$this->load->model('Tenancy/Inventory/Stock/Outgoing_Detail_model');
    ...
    {
        $format = $this->Common_Code_model->get_by_code_last('SERIES_OUTGOING');
        if ($format) {
            $counter = $this->Outgoing_model->get_series($format->name, $building_id, $date)->total + 1;
	...
	public function index()
	{
		...
		$data['results'] = $this->Outgoing_model->get_all([
        ...
        foreach ($data['results'] as $p) {
            // belongsTo
            $p->building = $this->Outgoing_model->building($p->building_id);
            // morph
            $p->to = $this->Outgoing_model->get_to($p);
            // hasMany
            $p->detail = $this->Outgoing_model->outgoing_detail($p->id);
            foreach ($p->detail as $u) {
                // belongsTo
                $u->item = $this->Outgoing_Detail_model->item($u->item_id);
            }
        }
		...
		$this->load->view('Tenancy/Inventory/Stock/Outgoing/index', $data);
	...
	{
        $user_id = $this->session->userdata('MIS_LOGGED_ID');
        $data['judul'] = 'Create Outgoing Stock';
		$data['bodyclass'] = 'skin-blue';
		$data['input_type'] = 'create';

        $data['building'] = $this->Building_model->get_all();
        $data['to_type'] = $this->Common_Code_model->get_by_code('PARTY');
        $data['result'] = null;
        $data['detail'] = null;

        $this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
        $this->load->view('Tenancy/Inventory/Stock/Outgoing/create', $data);
        $this->load->view('templates/footer');
	}
    ...
    public function store()
	{
        ...
        $this->form_validation->set_rules('to_type', 'Outgoing Party', 'required|numeric');
        $this->form_validation->set_rules('to_id', 'Outgoing To', 'required|numeric');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            redirect('Tenancy/Inventory/Stock/Outgoing/create');
        }
        ...
        try {
            ...
            $common = $this->Common_Code_model->find($this->input->post('to_type'));

            // Common_Code_model
            $this->Outgoing_model->insert([
                'series'  => $series[0],
            ...
            foreach($this->input->post('item') as $value){
                ...
                $this->Outgoing_Detail_model->insert([
                    'outgoing_id'  => $header_id,
                    'item_id'  => $item_id,
                    'qty'  => $qty,
                    'is_active'  => 1,
                    'username'  => $user_id,
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                redirect('Tenancy/Inventory/Stock/Outgoing/index');
            }
        } catch (Exception $e) {
            $this->db->trans_rollback();
	...
	{
        $user_id = $this->session->userdata('MIS_LOGGED_ID');
        $data['result'] = $this->Outgoing_model->get_by_id($id);
        ...
        $data['building'] = $this->Building_model->get_all();
        $data['to_type'] = $this->Common_Code_model->get_by_code('PARTY');
        $data['to_type_common'] = $this->Common_Code_model->get_by_name('PARTY', $data['result']->to_type);


        $data['detail'] = $this->Outgoing_model->outgoing_detail($data['result']->id);
        ...
        $this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar');
        $this->load->view('Tenancy/Inventory/Stock/Outgoing/edit', $data);
        $this->load->view('templates/footer');
	}
    ...
    public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            is_pesan('error','Unfornuately, something went wrong.');
            redirect('Tenancy/Inventory/Stock/Outgoing/edit/'.$id);
        }
        ...
        try {
            ...
            $detail = $this->Outgoing_Detail_model->get_by_outgoing_id($id);
            $this->Outgoing_Detail_model->delete_by_outgoing_id($id);
