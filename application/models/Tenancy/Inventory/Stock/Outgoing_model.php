...
{
    protected $table = 'tnc_inven_st_outgoing';
    ...
    public function get_series($code, $building_id, $date)
    {
        ...
        $query = $this->db->
            query("SELECT COUNT(*) AS total FROM tnc_inven_st_outgoing WHERE series_code = '$code' AND building_id = $building_id AND YEAR(date_trans) = $year AND MONTH(date_trans) = $month
        ")->row();
    ...
    // hasMany
    public function outgoing_detail($outgoing_id) {
        $this->load->model('Outgoing_Detail_model');
        return $this->Outgoing_Detail_model->get_by_outgoing_id($outgoing_id);
    }

    // morp
    public function get_to($data)
    {
        if ($data->to_type == 'DEPARTEMENT') {
            $this->load->model('departement_model');
            return $this->Departement_model->find($data->to_id);
        } elseif ($data->to_type == 'CUSTOMER') {
            $this->load->model('customer_model');
            return $this->Customer_model->find($data->to_id);
        } elseif ($data->to_type == 'SUPPLIER') {
            $this->load->model('supplier_model');
            return $this->Supplier_model->find($data->to_id);
        } elseif ($data->to_type == 'BUILDING') {
            $this->load->model('building_model');
            return $this->Building_model->find($data->to_id);
        } else {
            return null;
        }
    }
