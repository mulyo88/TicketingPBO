    ...
    <section class="content">
        ...
        <div class="panel">
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-6">
                        <form method="post" action="<?= site_url('Tenancy/MasterData/Letter_Line/update/'.$result->id) ?>">
                            <div class="box-body">
                                <div class="row">
                                    <div class="col">
                                            ...
                                            <div class="col-sm-10">
                                                <select name="party_type" id="party_type" class="form-control input-sm" onchange="selectParty()" required>
                                                    ...
                                                    <?php foreach ($common_code as $row): ?>
                                                        <option value="<?=$row->id?>" <?= $row->id == $result->party_type ? 'selected' : '' ?>><?=$row->name?></option>
                                                    <?php endforeach ?>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="position" name="position" placeholder="Position"
                                                    value="<?php echo $result->position; ?>" required
                                                >
                                            </div>
                                        </div>
                                    </div>
            ...
            success: function(data) {
                if (data) {
                    ...
                    for (let index = 0; index < data.length; index++) {
                        var selected = '';
                        if (data[index].id == result.party_id) {
                            selected = 'selected';
                        }

                        if (type == "TENANT") {
                            html += '<option value="' + data[index].id + '" ' + selected + '>' + data[index].brand + '</option>';
                        } else {
