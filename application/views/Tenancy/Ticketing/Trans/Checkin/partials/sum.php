...
<div class="row mt-2">
    ...
    <div class="col-sm-9">
        <input type="number" any="step" name="discount" id="discount" class="form-control <?= invalid5('discount') ?>" style="text-align: right;" placeholder="Discount" readonly required onkeyup="calculateQty()">
    </div>
</div>
    ...
    <div class="col-sm-9">
        <div class="input-group">
            <!-- <div class="input-group-text">
                <input type="checkbox" id="is_tax" name="is_tax" onclick="calculateQty()">
            </div> -->
            <input type="number" any="step" name="tax" id="tax" class="form-control <?= invalid5('tax') ?>" style="text-align: right;" placeholder="Tax" aria-label="tax" aria-describedby="basic-addon2" readonly required onkeyup="calculateQty()">
...
<div class="row mt-2">
    ...
    <div class="col-sm-9">
        <div class="input-group">
            <div class="input-group-text">
                <input type="checkbox" id="is_payment" name="is_payment" onclick="calculateQty()">
            </div>
...
<?php start_section('page-style-partials-sum'); ?>
    <style>
        
    </style>
<?php end_section('page-style-partials-sum'); ?>
    ...
    <script>
        ...
        function calculateQty() {
            ...
            if (rowCount) {
                for (let index = 0; index < rowCount.length; index++) {
                    ...
                    var xqty = parseFloat(xnumber(document.getElementById("cart[" + rows + "][qty]").value));
                    var rate = parseFloat(xnumber(document.getElementById("cart[" + rows + "][price]").value));
                    var xtotal = parseFloat(xqty) * parseFloat(rate);

                    document.getElementById("cart[" + rows + "][subtotal]").innerHTML = formatNumber(xtotal, 2);
                    total += parseFloat(xtotal);

                    tax_amount += parseFloat(xnumber(document.getElementById("cart[" + rows + "][tax_amount]").value)) * parseFloat(xqty);
                    discount_amount += parseFloat(xnumber(document.getElementById("cart[" + rows + "][discount_amount]").value)) * parseFloat(xqty);
                }
            }

            // if (document.getElementById("is_tax").checked) {
            //     document.getElementById("tax").value = formatDecimal(fn_i_tax, 2);
            // } else {
            ...
            // var tax = parseFloat(discount) + parseFloat(xnumber(document.getElementById("tax").value));
            document.getElementById("grandtotal").value = formatNumber(parseFloat(total) - parseFloat(discount_amount) + parseFloat(tax_amount), 2);

            if (document.getElementById("is_payment").checked) {
                document.getElementById("payment").value = xnumber(document.getElementById("grandtotal").value);
            }
        ...
        function generate_party() {
            ...
            $.ajax({
                dataType: "json",
                type: "GET",
                url: "<?= site_url('Tenancy/API/GlobalAPI_Payment_Gate/setup_payment_gate') ?>/" + method_id + "/" + party_id + "/" + building_id + "/" + value + "/" + type,
