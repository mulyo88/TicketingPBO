...
<?php include_view('Tenancy/Ticketing/Trans/Checkout/partials/modal/gate'); ?>
...
<div class="content-wrapper">
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="text-capitalize active"><a href="<?= site_url('Tenancy/Ticketing/Trans/Checkout/index') ?>"><?php echo $judul ?></a></li>
        </ol>
    </section>
    ...
    <section class="content">
        ...
        <form method="GET" action="<?= site_url('Tenancy/Ticketing/Trans/Checkout/index') ?>">
            <div class="panel">
                        ...
                        <table id="tbl_checkout" class="table table-sm table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center text-capitalize">checkin date</th>
                                    <th class="text-center text-capitalize">checkout date</th>
                            ...
                            <tbody>
                                <?php foreach ($results as $row): ?>
                                    <tr>
                                        ...
                                        <td class="text-capitalize text-center"><?= $row->checkin_date ? date("d-M-Y H:i:s", strtotime($row->checkin_date)) : '' ?></td>
                                        <td class="text-capitalize text-center"><?= $row->checkin_scan_date ? date("d-M-Y H:i:s", strtotime($row->checkin_scan_date)) : '' ?></td>
                                        <td class="text-capitalize text-center"><?= $row->checkout_date ? date("d-M-Y H:i:s", strtotime($row->checkout_date)) : '' ?></td>
                                        <td class="text-capitalize"><?= $row->barcode ?></td>
                                        <td class="text-capitalize"><?= $row->name ?></td>
                                        <td export=false class="text-center">
                                            <a onclick="return confirm('Are you sure?')" href="<?= site_url('Tenancy/Ticketing/Trans/Checkout/destroy/'.$row->checkin_id.'/'.$row->ticket_id.'/'.$row->seq.'/'.$row->gate_id); ?>" class="label label-danger" style="margin-left: 5px;">
