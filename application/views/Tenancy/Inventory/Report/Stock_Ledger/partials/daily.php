    ...
    <script>
        function load_daily() {
            ...
            $.ajax({
                dataType: "json",
                type: "GET",
                url: "<?= site_url('Tenancy/API/GlobalAPI_Trans/report_stock_daily') ?>/" + date_trans + "/" + area + "/" + departement + "/" + category + "/" + code + "/" + name + "/" + seq_uom,
                ...
                success: function(data) {
                    document.getElementById("panel-data").innerHTML = fill_daily(data, date_trans);
                    if (data.length > 0) {
                        select_mode();
                    }
                },
        ...
        function fill_daily(data, date_trans) {
            if (data.length == 0) {
                set_card(0, 0, 0, 0, 0);
                ...
                var html = '';
                html += '<table id="tbl-daily" class="table table-bordered table-striped" style="table-layout: fixed; width: 100%;">';
                    html += '<colgroup>';
                        ...
                        // daily column
                        for (let i = 1; i <= 31; i++) {
                            html += '<col style="width:80px;">'; //daily Inven
                            html += '<col style="width:80px;">'; //daily Adj
                            html += '<col style="width:80px;">'; //daily In
                            html += '<col style="width:80px;">'; //daily Out
                            html += '<col style="width:80px;">'; //daily Total
                            html += '<col style="width:80px;">'; //daily Stock
                        }
                    html += '</colgroup>';
                    ...
                    html += '<thead style="background-color: #B3B3B3; color: black; position:sticky; top:0; z-index:40;">';
                        html += '<tr style="position:sticky; top:0; z-index:41;">';
                            ...
                            var yesterday = new Date(date_trans);
                            yesterday.setDate(yesterday.getDate() - 1);

                            html += '<th colspan="6" style="text-align: center; vertical-align: middle; background-color: #EDD861;" class="colspan-beginning">Beginning ' + moment(yesterday).format('DD-MMM') + '</th>';
                            ...
                            for (let day = 1; day <= 31; day++) {
                                ...
                                html += '<th colspan="6" style="text-align: center; vertical-align: middle; ' + color + '" class="colspan-day">' + moment(dt).format('DD-MMM') + '</th>';
                            }
                        html += '</tr>';
        ...
        function export_daily() {
            let table = document.getElementById("tbl-daily");
            ...
            a.download = "daily_" + moment(new Date()).format('DD_MM_YYYY_HH_mm_ss') + ".xls";
            a.click();
        }
    </script>
