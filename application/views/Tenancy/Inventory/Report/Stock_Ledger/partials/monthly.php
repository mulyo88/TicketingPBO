    ...
    <script>
        function load_monthly() {
            ...
            $.ajax({
                dataType: "json",
                type: "GET",
                url: "<?= site_url('Tenancy/API/GlobalAPI_Trans/report_stock_monthly') ?>/" + date_trans + "/" + area + "/" + departement + "/" + category + "/" + code + "/" + name + "/" + seq_uom,
                ...
                success: function(data) {
                    document.getElementById("panel-data").innerHTML = fill_monthly(data, date_trans);
                    if (data.length > 0) {
                        select_mode();
                    }
                },
        ...
        function fill_monthly(data, date_trans) {
            if (data.length == 0) {
                set_card(0, 0, 0, 0, 0);
                ...
                var html = '';
                html += '<table id="tbl-monthly" class="table table-bordered table-striped" style="table-layout: fixed; width: 100%;">';
                    html += '<colgroup>';
                        ...
                        html += '<col style="width:80px;">';  // MinStock
                        html += '<col style="width:80px;">';  // MaxStock
                        ...
                        // monthly column
                        for (let i = 1; i <= 31; i++) {
                            html += '<col style="width:100px;">'; //monthly Inven
                            html += '<col style="width:100px;">'; //monthly Adj
                            html += '<col style="width:100px;">'; //monthly In
                            html += '<col style="width:100px;">'; //monthly Out
                            html += '<col style="width:100px;">'; //monthly Total
                            html += '<col style="width:100px;">'; //monthly Stock
                        }
                    html += '</colgroup>';
                    ...
                    html += '<thead style="background-color: #B3B3B3; color: black; position:sticky; top:0; z-index:40;">';
                        html += '<tr style="position:sticky; top:0; z-index:41;">';
                            ...
                            var lastDay = new Date(date_trans);
                            lastDay.setMonth(lastDay.getMonth() -1);
                            lastDay = new Date(lastDay.getFullYear(), lastDay.getMonth() + 1, 0);

                            html += '<th colspan="6" style="text-align: center; vertical-align: middle; background-color: #EDD861;" class="colspan-beginning">Beginning ' + moment(lastDay).format('YYYY-MMM-DD') + '</th>';
                            ...
                            for (let day = 1; day <= 12; day++) {
                                ...
                                html += '<th colspan="6" style="text-align: center; vertical-align: middle;" class="colspan-day">' + moment(dt).format('YYYY-MMM') + '</th>';
                            }
                        html += '</tr>';
        ...
        function export_monthly() {
            let table = document.getElementById("tbl-monthly");
            ...
            a.download = "monthly_" + moment(new Date()).format('DD_MM_YYYY_HH_mm_ss') + ".xls";
            a.click();
        }
    </script>
