<?php start_section('page-script-datalist-simple'); ?>
    <script>
        function load_datalist_simple() {
            ...
            var html = '';
            html += '<table id="tbl_pos_datalist_simple" class="table table-bordered table-striped">';
                html += '<thead style="background-color: #B3B3B3; color: black; position: sticky; top: 0; z-index: 10;">';
                    html += '<tr style="position: sticky; top: 0; z-index: 11;">';
                        ...
                        html += '<th class="text-center text-capitalize">party</th>';
                ...
                html += '<tbody>';
                    for (let index = 0; index < results.length; index++) {
                        html += '<tr>';
                            ...
                            html += '<td class="text-capitalize text-right">' + formatNumber(results[index].grandtotal, 2) + '</td>';
                            html += '<td class="text-capitalize">' + results[index].methode + '</td>';

                            if (results[index].party_type == "BANK") {
                                html += '<td class="text-capitalize">' + (results[index].party ? results[index].party.account_name : "") + '</td>';
                            } else if (results[index].party_type == "PAYMENT_GATE") {
                                html += '<td class="text-capitalize">' + (results[index].party ? results[index].party.name : "") + '</td>';
                            } else {
                                html += '<td class="text-capitalize"></td>';
                            }
        ...
        function chart_datalist_simple() {
            if (transChart) {
                transChart.destroy();
            }
