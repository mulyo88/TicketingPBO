...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <form method="GET" action="<?= site_url('Tenancy/Inventory/Stock/Incoming/index') ?>">
            <div class="panel">
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table id="tbl_incoming" class="table table-sm table-bordered">
                            <thead>
                                <tr>
                                    <th export=false></th>
                                    <th class="text-center text-capitalize">series</th>
                                    <th class="text-center text-capitalize">trans date</th>
                                    <th class="text-center text-capitalize">from</th>
                                    <th class="text-center text-capitalize">to area</th>
                            ...
                            <tbody>
                                <?php foreach ($results as $row): ?>
                                    <tr>
                                        ...
                                        <?php if ($row->from_type == "DEPARTEMENT") { ?>
                                            <td class="text-capitalize"><?= $row->from ? 'Dept - ' .$row->from->code. ' - ' . $row->from->name : '' ?></td>
                                        <?php } else if ($row->from_type == "CUSTOMER") { ?>
                                            <td class="text-capitalize"><?= $row->from ? 'Cust - ' . $row->from->name : '' ?></td>
                                        <?php } else if ($row->from_type == "SUPPLIER") { ?>
                                            <td class="text-capitalize"><?= $row->from ? 'Supp - ' . $row->from->name : '' ?></td>
                                        <?php } else if ($row->from_type == "BUILDING") { ?>
                                            <td class="text-capitalize"><?= $row->from ? 'Area - ' . $row->from->code . ' - ' . $row->from->name : '' ?></td>
                                        <?php } else { ?>
                                        ...
                                        <td export=false class="text-center">
                                            <a href="<?= site_url('Tenancy/Inventory/Stock/Incoming/edit/'.$row->id); ?>" class="label label-warning">
                                                <i class="fa fa-edit"></i><span style="margin-left: 5px;">Edit</span>
                                            </a>

                                            <a onclick="return confirm('Are you sure?')" href="<?= site_url('Tenancy/Inventory/Stock/Incoming/destroy/'.$row->id); ?>" class="label label-danger" style="margin-left: 5px;">
...
<script type="text/javascript">
    $(document).ready(function(){
        // reset state old
        if ($.fn.DataTable.isDataTable('#tbl_incoming')) {
            $('#tbl_incoming').DataTable().clear().destroy();
        }

        // init DataTables
        var table = $('#tbl_incoming').DataTable({
            stateSave: false,
            serverSide: false,
            "columnDefs": [
                {
        ...
        // remove query string tbl_incoming_length from URL
        if (window.location.search.includes('tbl_incoming_length')) {
            let url = new URL(window.location);
            url.searchParams.delete('tbl_incoming_length');
        ...
        $('#tbl_incoming tbody').on('click', 'td.dt-control', function () {
