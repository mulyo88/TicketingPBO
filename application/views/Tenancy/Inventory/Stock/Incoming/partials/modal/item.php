    ...
    <script type="text/javascript">
        ...
        function add_item(data, type="select") {
            ...
            if (type == "select") {
                ...
                var id = data.item_id;
                var name = data.item ? data.item.KdBarang + ' - ' + data.item.NmBarang : 'not found';
                var qty = formatDecimal(data.qty, 0);
            }
            ...
            html +='<tr class="rowCount-item" id="rowCount-item_' + rows + '">';
                ...
                html +='<td>';
                    html +='<input type="number" any="step" id="item[' + rows + '][qty]" name="item[' + rows + '][qty]" class="form-control qty" min="1" value="' + qty + '" style="text-align: right;" onkeyup="calculateQty()">'
                html +='</td>';
        ...
        function reset_item() {
            document.getElementById("tbody-item").innerHTML = '<tr><td class="text-center text-capitalize text-muted" colspan="3">no data record</td></tr>';
            calculateQty();
        }
