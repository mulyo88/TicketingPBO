    ...
    <section class="content">
        ...
        <div class="panel">
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-6">
                        <form method="post" action="<?= site_url('Tenancy/Inventory/MasterData/Payment_Gate/update/'.$result->payment_gate_id.'/'.$result->key_id) ?>">
                            <div class="box-body">
                                <div class="row">
                                    <div class="col">
                                            ...
                                            <div class="col-sm-10">
                                                ...
                                                >
                                                    <?php foreach ($payment_gate_type as $row): ?>
                                                        <option value="<?=$row->id?>" <?= old('key_id') ? (old('key_id') == $row->id ? 'selected' : '') : ($result->key_id == $row->id ? 'selected' : '') ?>><?=$row->name?></option>
                                                    <?php endforeach ?>
                                                </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="key" name="key" placeholder="Key" required
                                                    <?= invalid('key') ?>
                                                    value="<?php echo old('key') ? old('key') : $result->decrypt; ?>"
                                                >
                                                <?= error('key') ?>
                                            </div>
                                        </div>
